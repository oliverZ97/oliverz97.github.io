export function isMoreThanADay(date1: Date, date2: Date) {
 ...
 export function sameDate(date1: Date, date2: Date) {
    ...
    export function getImgSrc(name: string) {
       ...
       const basepath = import.meta.env.VITE_ENVIRONMENT === "dev" ? "/src/assets/characters/" : "assets/characters/"
 
       return basepath + filename + ".webp"
    }

    export function compareObjects<T extends Record<string, any>>(obj1: T, obj2: T): {
      all: string[],
      short: string[]
   } {
      const sameFieldsObj: {
