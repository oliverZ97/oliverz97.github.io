...
export default function Calendar({
    ...
    const [open, setOpen] = useState(ignoreOpener ? true : false);
    ...
    const handleClose = () => {
        ...
        if (closeDrawer && closeOnCalendarClose) {
            closeDrawer();
        }
    };
    ...
    return (
        <>
            ...
            {(open || ignoreOpener) && (
                <Dialog
                    open={open || Boolean(ignoreOpener)}
                    onClose={handleClose}
                >
                    <Box sx={{ padding: 2, backgroundColor: COLORS.quiz.background, color: "white" }}>
                            ...
                            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 1 }}>
                                ...
                                {Array.from({ length: daysInMonth }, (_, i) => (
                                    ...
                                    >
                                        {i + 1}
                                        <Box>
                                            <Typography fontWeight={"bold"} color={COLORS.quiz.light_red}>
                                                {data[DateTime.local(selectedDate.year, currentMonth, i + 1, 0).toISODate() || '']?.value}
                                            </Typography>
                                        </Box>
                                    </Box>
                                ))}
