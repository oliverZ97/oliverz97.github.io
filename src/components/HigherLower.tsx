...
export const HigherLower = ({ charData, animeData }: HigherLowerProps) => {
  ...
  const [activeElement, setActiveElement] = useState<Character | Anime | null>(null);
  const [newElement, setNewElement] = useState<Character | Anime | null>(null);
  const [secondNextElement, setSecondNextElement] = useState<Character | Anime | null>(null);
  ...
  function updateScores(score: Score) {
    const highscores = getHighscoresFromProfile(
      mode === "height" ? QUIZ_KEY.HIGHERLOWER_HEIGHT : QUIZ_KEY.HIGHERLOWER_ANIME,
    );
  ...
  function init() {
    ...
    if (mode === "height") {
      ...
      const yetAnotherRandomChar = filteredChars[Math.floor(Math.random() * filteredChars.length)];
      ...
      const anotherRandomAnime = animeData[Math.floor(Math.random() * animeData.length)];
      setNewElement(anotherRandomAnime);
      const yetAnotherRandomAnime = animeData[Math.floor(Math.random() * animeData.length)];
  ...
  function handleCharacterChange() {
    ...
    const filteredChars = charData.filter((char) => char.Height !== null);
    const anotherRandomChar = filteredChars[Math.floor(Math.random() * filteredChars.length)];
    if (anotherRandomChar.id === activeElement?.id || anotherRandomChar.id === newElement?.id) {
      //avoid duplicates
  ...
  function handleAnimeChange() {
    ...
    const randomAnime = animeData[Math.floor(Math.random() * animeData.length)];
    if (randomAnime.id === newElement?.id || randomAnime.id === secondNextElement?.id) {
      //avoid duplicates
      handleAnimeChange();
      return;
    }
    ...
    if (activeElement && newElement && "Height" in activeElement && "Height" in newElement) {
      if (guess === "higher" && newElement.Height! > activeElement.Height!) {
        ...
        saveFieldToTotalStatistics(
          ...
          (getStatisticField(StatisticFields.higherlowerHeightPointsTotal) ?? 0) + (points ?? 0),
        );
  ...
  function checkAnimeReleaseYearGuess(guess: "higher" | "lower") {
    if (
      ...
      if (
      ...
      } else if (newElement.First_Release_Year! === activeElement.First_Release_Year!) {
        setPoints(points + 1);
        // Trigger animation
        setIsAnimating(true);
        ...
        saveFieldToTotalStatistics(
          ...
          (getStatisticField(StatisticFields.higherlowerAnimePointsTotal) ?? 0) + (points ?? 0),
        );
        updateScores(scoreObj);
        init();
        return;
  ...
  return (
    <Box>
      ...
      >
        {score && (
          ...
          >
            ...
            <Typography fontSize={"12px"}>{"Points: " + score.points}</Typography>
            <Typography fontSize={"12px"}>{"Date: " + score.date}</Typography>
          </Box>
        )}
        <Typography sx={{ fontWeight: "bold", fontSize: "28px", color: "white" }}>
          <Typography
            sx={{
      ...
      >
        ...
        >
          <Typography sx={{ color: "white", fontSize: "24px", paddingLeft: "2px" }}>{`${String(
            points,
        ...
        >
            ...
            <Box sx={{ position: "relative", width: "300px", height: "400px" }}>
              ...
              {newElement && (
                <Box
                  ...
                  src={
                    mode === "height" ? getImgSrc(newElement?.id) : getAnimeImgSrc(newElement?.id)
                  }
                />
              )}
          ...
          {showGameOver && (
            ...
            >
              <Typography sx={{ color: "white", fontSize: "32px", fontWeight: "bold" }}>
                Game Over
              </Typography>
              <Typography sx={{ color: "white", fontSize: "24px", marginTop: 2 }}>
                Your final score: {points}
              </Typography>
              <Button variant="contained" sx={{ marginTop: 3 }} onClick={() => init()}>
                Restart
              </Button>
            </Box>
          )}
