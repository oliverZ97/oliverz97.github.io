...
export const CardInfoEntry = ({ card, text, isFullArt, size = 1 }: CardInfoEntryProps) => {
  ...
  const left = isFullArt
    ? CARD_INFO_ENTRY_BASE.LEFT_FULL_ART * size
    : CARD_INFO_ENTRY_BASE.LEFT_DEFAULT * size;
  ...
  const fontSize =
    (isLongText ? CARD_INFO_ENTRY_BASE.FONT_SIZE_LONG : CARD_INFO_ENTRY_BASE.FONT_SIZE_NORMAL) *
    size;
    ...
    <Box sx={{ display: "flex", position: "absolute", top: top, left: left }}>
      ...
      >
        {isFullArt && (
          <Box
            sx={{
              background: `linear-gradient(90deg,${darken(COLORS.cards.rare, 0.3)} 0%, ${
                COLORS.cards.rare
              } 50%, ${darken(COLORS.cards.rare, 0.3)} 100%)`,
              position: "absolute",
              minWidth: "100%",
              height: "100%",
              borderRadius: borderRadius,
