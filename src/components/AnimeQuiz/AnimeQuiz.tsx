...
import { getDailyUTCDate, getRandomAnime, hasBeenSolvedToday } from "common/utils";
...
export const AnimeQuiz = ({ animeData, endlessMode }: AnimeQuizProps) => {
    ...
    function init() {
        ...
        let target = getRandomAnime(animeData, endlessMode ? true : false);
        if (endlessMode) {
        ...
        if (value && targetAnime) {
            ...
            if (res.all.length + 1 === Object.keys(targetAnime).length) {
                ...
                if (!endlessMode) {
                    ...
                    localStorage.setItem("animequiz_HasBeenSolvedToday", utcDate.toISOString());
                }
                if (points > 0) {
                    //Set Highscore
                    const scoreObj = {
