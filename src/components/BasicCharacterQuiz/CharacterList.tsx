...
export default function CharacterList({ searchHistory, targetChar }: CharacterListProps) {
   ...
   return (
      <Box>
         <Box sx={{ marginTop: 4, display: "flex", flexDirection: "column", justifyContent: "flex-start", maxHeight: "400px", overflowX: "hidden", overflowY: "auto", borderTopLeftRadius: "16px", borderTopRightRadius: "16px", border: `1px solid ${COLORS.quiz.light}`, borderBottom: 0, backgroundColor: COLORS.quiz.secondary }}>
            ...
            {/* Data Rows */}
            {searchHistory.map((item) => <Box key={item.Name} sx={{ display: 'grid', gridTemplateColumns: '60px repeat(8, 1fr)', gap: 2, paddingX: 2, marginBottom: 2 }}>
