...
export const AvatarEdit = forwardRef((props, ref) => {
    ...
    function handleColorSelect(color: string) {
        ...
        if (profile) {
            setUserProfile({
                ...profile,
                avatar: {
                    ...profile.avatar || { avatarUrl: "chisato.png", backgroundColor: COLORS.profile.backgroundColors[0] },
    ...
    return (
        <Box height={"100%"} display={"flex"} maxWidth={400} flexDirection={"row"} alignItems={"flex-start"}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, marginRight: 2 }}>
                <Avatar ref={avatarRef} size={120} />
                {!showEdit && username !== "Guest" && <IconButton onClick={() => setShowEdit(true)}>
                    <EditIcon sx={{ color: "white" }} />
                </IconButton>}
                {showEdit && <IconButton onClick={() => handleSave()}>
            ...
            <Box>
                {showEdit && (
                    <Fade in={showEdit} timeout={500} unmountOnExit>
                        <Box>
                            <Box sx={{ marginTop: 2, display: "flex", gap: 1, flexWrap: "wrap", overflowX: "scroll" }}>
                                <Box sx={{
                                    display: "flex",
                                    gap: 1,
                                    flexWrap: "wrap",
                                    ...
                                    {COLORS.profile.backgroundColors.map((color) => (
                                        <Box onClick={() => handleColorSelect(color)}
                                            key={color.split("#")[1]}
                                            sx={{
                                                backgroundColor: color,
                                                height: 33,
